...
def getLocation(ad: selenium.webdriver.remote.webelement.WebElement):
    geo = ad.find_element_by_class_name("clad__geo")
    divs = geo.find_elements_by_tag_name("div")
    region = divs[1].text
    city = divs[1].text
    try:
        place = divs[2].text
...
def getCellphone(constants, driver: selenium.webdriver) -> str:
    try:
        cellphone = driver.find_element_by_id("phone_link_bottom")
...
def isVerified(ad: selenium.webdriver.remote.webelement.WebElement) -> str: 
    try:
        ad.find_element_by_class_name("verified-badge")
        return "1"
    except: 
        return "0"
...
def isFeature(ad: selenium.webdriver.remote.webelement.WebElement) -> str:
    try:
        ad.find_element_by_class_name("label-badge-featured")
        return "1"
    except: 
        return "0"
